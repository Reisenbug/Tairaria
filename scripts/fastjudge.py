...
QUESTIONS = {
    ...
    # 目标要做几次。只读目标文本不判达成:数次数是算术归代码,读意图才是判断
    "goal_quantity": {
        "counted": Noul(
            instructions="这个目标带明确的次数或数量要求(砍两棵、挖十个铁、开三个箱子)",
        ),
        "how_many": Choice(
            instructions="目标要求做多少次这件事",
            criteria={
                "1": "一次就够", "2": "两次", "3": "三次", "4": "四次", "5": "五次",
                "6_to_10": "六到十次", "more_than_10": "十次以上",
                "until_enough": "没说次数,攒够某个量为止",
                "unspecified": "完全没提数量",
            },
        ),
    },
